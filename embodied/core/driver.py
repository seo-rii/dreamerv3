...
class Driver:
    ...
    def __init__(self, make_env_fns, parallel=True, vector_env=1, **kwargs):
    ...
    def reset(self, init_policy=None):
        self.acts = {
            k: np.zeros((self.length, self.vector_env,) + v.shape, v.dtype)
            for k, v in self.act_space.items()}
        self.acts['reset'] = np.ones((self.length, self.vector_env,), bool)
    ...
    def _step(self, policy, step, episode):
        ...
        obs = {k: obs[k].reshape((obs[k].shape[0] * obs[k].shape[1],) + obs[k].shape[2:]) for k in obs.keys()}
        ...
        acts = {k: v.reshape((self.length, self.vector_env,) + v.shape[1:]) for k, v in acts.items()}
        self.acts = acts
        trans = {**obs, **acts, **outs}
